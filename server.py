...
class _TurboEncoder:
    ...
    def __init__(self, mon: dict, fps: int, kbps: int):
        ...
        bufsize_k = self.kbps
        # [2026-08-31 화질우선 재편 — 대표님 실사용 판정 "글씨를 읽을 수 없다"] 해상도 축소는 4K 글씨를
        # 뭉개므로(1280 단계가 주범) 최후의 최후로만: 사다리 바닥(≤1200k)에서만 1920, 그 위는 항상 원본 4K.
        # 혼잡 대응의 주력은 fps 자동 사다리(폰, 글씨에 무해 — fps 낮으면 프레임당 비트가 늘어 오히려 선명).
        if self.kbps <= 1200:
            _tw = 1920
        else:
            _tw = self.w
